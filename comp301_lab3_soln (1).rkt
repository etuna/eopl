...
;; solution starts
(define-datatype variants variants?
  (variants-exp
   (exp1 expression?)
   (exp2 expression?)
...
;; solution starts
(define value-of-variants
  (lambda (varints env)
    (cases variants varints
      (variants-exp (exp1 exp2 other-variants)
                    (if (expval->bool (value-of exp1 env))
                        (value-of exp2 env)
                        (value-of-variants other-variants env)))
      (empty-variants-exp ()
                          (eopl:printf "None of the conditions are true!")))))
;; solution ends
...
; raises an error - cases: not a expval: #<void>
(eopl:printf "~a\n" (number->string (expval->num (run "let x = 6 in let y = 5 in cond {zero?(-(x,35)) ==> -(x,2)}{zero?(-(y,4)) ==> -(y,34)} end"))))
