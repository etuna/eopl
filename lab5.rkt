#lang eopl
;The LETREC language below is the language you have seen in class, that computes expressions like the following one:
;
;letrec double(x)
...
; The parts of code you will need to modify have been marked with "your modification goes here"
; After your modification test your code with the following command:
;(eopl:printf "~a\n" (run "letrec
;        even(x) = if zero?(x) then 1 else (odd -(x, 1))
;        odd(x) = if zero?(x) then 0 else (even -(x, 1))
;      in (odd 13)"))
...
;;your modification goes here (hint: use arbno)---
(define grammar
...
(eopl:printf "~a\n" (run "letrec
        double(x) = if zero?(x) then 0 else -((double -(x, 1)), -(0, 2))
      in (double 6)"))